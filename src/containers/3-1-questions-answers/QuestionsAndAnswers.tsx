...
import {fetchList, updateRemark} from './questions-answers.action'
...
class QuestionsAndAnswers extends React.Component<QuestionsAndAnswersProps> {
  ...
  handleExportExcel = () => {
    exportExcel('/export/question/order/list', {
  ...
  updateRemark = (newRemark) => {
    ...
    this.props.updateRemark(item['question_order_code'], newRemark)
  }

  updateIsHideState = (orderCode) => {
    this.props.updateIsHideState(orderCode)
  }
  ...
  componentWillReceiveProps(nextProps: QuestionsAndAnswersProps) {
...
export default connect(mapStateToProps, {fetchList, updateRemark})(
  addCommonFunction(QuestionsAndAnswers)
)
