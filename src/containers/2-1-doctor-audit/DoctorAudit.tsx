...
import {fetchList, updateRemark, updateDoctor} from './doctor-audit.action'
...
class DoctorAudit extends React.Component<DoctorAuditProps> {
  ...
  componentWillReceiveProps(nextProps: DoctorAuditProps) {
    ...
    if (!this.props.updateDoctorSuccess && nextProps.updateDoctorSuccess) {
      this.props.showSuccess('更新医生信息成功！')
      this.toPage()
    }
      ...
      <div className="app-function-page">
        {
          this.state.showEdit && (
            <UpdateDoctorDialog
              updateDoctor={(options) => this.updateDoctor(options)}
              doctor={item}
...
export default connect(mapStateToProps, {fetchList, updateRemark, updateDoctor})(addCommonFunction(DoctorAudit))
